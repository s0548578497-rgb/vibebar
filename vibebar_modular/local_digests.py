"""Shared local digest generation; publishing remains an explicit socket."""
...
from .contracts import Clock, CommandResult, CommandRunner, DigestSocket


@dataclass(frozen=True, slots=True)
class LocalDigestSocket:
    root: Path
    digest_dir: Path
    journal: Path
    ...
    def build_week(self, end: date | None = None) -> CommandResult:
        ...
        result = self.runner.run((str(self.root / "bin" / "vibebar-week.py"), str(self.journal), last_day.isoformat()))
    ...
    def _with_path(result: CommandResult, path: Path) -> CommandResult:
        return result if not result.succeeded else CommandResult(0, str(path), result.stderr)
