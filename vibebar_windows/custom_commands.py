"""Persistent user-defined voice command aliases."""
...
class NullCustomCommandStore:
    """Sealed fallback: exposes no commands and never writes user data."""
...
class CustomCommandStore:
    ...
    def delete(self, phrase: str) -> None:
        rows = [row for row in self.load() if row.phrase != phrase]
        self._save(rows)

    def aliases(self) -> dict[str, tuple[str, ...]]:
        return {
            kind: tuple(row.phrase for row in self.load() if row.kind == kind)
